...
status_t ISurfaceTexture::QueueBufferInput::flatten(
        void*& buffer, size_t& size, int*& fds, size_t& count) const
{
    ...
    if (haveFence) {
        size_t size_tmp = size - (p - (char*)buffer);
        err = fence->flatten((void*&)p, size_tmp, fds, count);
    }
    return err;
}

status_t ISurfaceTexture::QueueBufferInput::unflatten(
        void const*& buffer, size_t& size, int const*& fds, size_t& count)
{
    ...
    if (haveFence) {
        fence = new Fence();
        size_t size_tmp = size - (p - (char*)buffer);
        err = fence->unflatten((void const*&)p, size_tmp, fds, count);
    }
    return err;
}
