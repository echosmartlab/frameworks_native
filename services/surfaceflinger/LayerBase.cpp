...
void LayerBase::computeGeometry(const sp<const DisplayDevice>& hw, LayerMesh* mesh) const
{
    ...
    // TODO:Don`t transform for all native surface
    bool isBootAnimation = ( 13 == strlen(mName.string()) ) && (0 == strcmp(mName.string(),"BootAnimation"));
    const Transform tr( isBootAnimation?s.transform:(hw->getTransform()*s.transform));
